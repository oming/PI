...
class LoginGUI extends JFrame implements ActionListener {
	...
	// 기본 초기
	LoginGUI(String host, int port) {
		this.setLayout(new FlowLayout());
		// The NorthPanel with:
		JPanel northPanel = new JPanel(new GridLayout());
		// the server name and the port number
		JPanel serverAndPort = new JPanel(new GridLayout(3, 2));
		...
		serverAndPort.add(new JLabel("Board Title:"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		/*
		 * if(o == logout) { client.sendMessage(new
		 * ChatMessage(ChatMessage.LOGOUT, "")); return; } // if it the who is
		 * in button if(o == whoIsIn) { client.sendMessage(new
		 * ChatMessage(ChatMessage.WHOISIN, "")); return; }
		 */
		...
		if (o == login) {
			...
			client = new Client("localhost", 1500, "loask");
